function [denoised_data, noise_score, predicted_score] = score_2020_exp(raw_data, sampling_frequency, line_frequency, cycles_per_template, calc_score_flag, remove_noise_flag, threshold_score)
    ...
    NUM_HARMONICS_SCALING_CONSTANTS = [ 1 1 1 1];
...
if(calc_score_flag)
    ...
    filtered_data_index = 1;
    SCALING_CONSTANT = 254 * SCORE_SCALING_CONSTANT;
    for i = 1:cycles_in_grid
        start_idx = floor(filtered_data_index);
        end_idx = floor(filtered_data_index + samps_per_cycle - 1);
%         median_of_cycle = median(highpassed_data(floor(filtered_data_index):floor(filtered_data_index + samps_per_cycle - 1)));
